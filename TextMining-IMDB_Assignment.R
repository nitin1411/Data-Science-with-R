...
corp_clean<-tm_map(corp_clean, removeWords, c("gameofthrones")) ## clean some words
...
tdm <- TermDocumentMatrix(corp_clean)
dtm <- t(tdm)
tdm <- as.matrix(tdm)
# Bar plot
w <- rowSums(tdm)
...
corp_clean <- tm_map(corp_clean, removeWords, c('apple','air','can','mcds','macbook','product','windows'))
corp_clean <- tm_map(corp_clean, stripWhitespace)
tdm <- TermDocumentMatrix(corp_clean)
tdm <- as.matrix(tdm)
...
ex<-get_sentences(txt)
nrc_data<-get_nrc_sentiment(ex)
...
sentiment_vector<-get_sentiment(ex,method="bing")
sentiment_afinn<-get_sentiment(ex,method="afinn")
sentiment_nrc<-get_sentiment(ex,method="nrc")
